  ...
  var Init = {
    ...
    passengerBox: function () {
      if($('.pessenger-box').length){
        ...
        $('.seat-booking').on('click',function(e){
          e.preventDefault();
          e.stopPropagation();
          var $dropdown = $(this).closest('.custom-sel-input-block').find('.pessenger-box');
          
          // Hide all other dropdowns
          $('.pessenger-box').not($dropdown).hide();
          
          // Toggle this dropdown
          $dropdown.toggle();
          
          console.log('Passenger dropdown clicked, visibility:', $dropdown.is(':visible'));
        });

        // Click outside to hide dropdown
        $(document).on('click', function(event) {
          var isClickInsideDiv = $div.is(event.target) || $div.has(event.target).length > 0;
          var isClickInsideButton = $(event.target).is('.seat-booking') || $(event.target).closest('.seat-booking').length > 0;
        ...
        // Update passenger count on increment
        $('.pessenger-box .increment').on('click',function(e){
          e.stopPropagation();
          var $area = $(this).closest('.passenger-area');
          if ($area.length) {
            var total = 0;
            $area.find('.quantity-wrap input.number').each(function(){ 
              total += parseInt($(this).val(), 10) || 0; 
            });
            $area.closest('.custom-sel-input-block').find('.total-pasenger').text(total);
          }
        });

        // Update passenger count on decrement
        $('.pessenger-box .decrement').on('click',function(e){
          e.stopPropagation();
          var $area = $(this).closest('.passenger-area');
          if ($area.length) {
            var total = 0;
        ...
        $('.pessenger-box .radio-button').on('click',function(e){
          e.stopPropagation();
          var $area = $(this).closest('.passenger-area');
          if ($area.length) {
            $area.closest('.custom-sel-input-block').find('.pasenger-class').text($(this).val());
          }
        });
      }
    },
    ...
    timepicker:function(){
      $(document).ready(function () {
        if (typeof $.fn.timepicker === 'function' && $('.timepicker').length) {
    ...
    contactForm: function () {
      $(".contact-form").on("submit", function (e) {
        // Tum contact formlar artik ASP.NET Core ile calisiyor
        // PHP endpoint kaldirildi, normal form submit kullaniliyor
        if (!$(this).valid()) {
          e.preventDefault();
          return false;
        }
        // Form gecerliyse normal submit'e izin ver
      });
    },
